...
class ReflectionInstrumenter(using State, Raise, Ctx) extends BlockTransformer(SymbolSubst.Id):
  ...
  def stageMethods(ownerSym: DefinitionSymbol[?], modSym: InnerSymbol, forClass: Bool, cacheNme: Str, generatorMapNme: Str)(methods: Ls[FunDefn]): (FunDefn, Ls[FunDefn], Block => Block) =
    ...
    val webdemoEntryFunDef =
        ...
        FunDefn.withFreshSymbol(S(modSym), sym, PlainParamList(Nil) :: Nil, genShowBody())(false, N, Visibility.Public)

    (entryFunDef, webdemoEntryFunDef :: propFunDef :: stagedMethods ++ generatorMethods, b => cacheDecl(generatorMapDecl(b)))
